...
% End trial
	...
	% TODO: why do any of these need to be signals (or rather, why aren't they in trialData)
	% TODO: move all non-dynamic-within-trial signals to trialData
	...
	preStimQuiescence = sig.quiescenceWatch(prestimQuiescentPeriod, t, wheel, quiescThreshold);  % currently noop via Inf, waste of processing
	...
	% TODO: partial reward tracker
	stimDisplacement = wheelGain*millimetersFactor*(wheel - wheel.at(interactiveOn));

	% TODO: turn into window
	% TODO: add second window for partial reward
	...
	responseData = stimDisplacement; % why is this needed?
	% Update performance
	% trialData = responseData.at(response).scan(@updateTrialData,trialData).subscriptable;
	trialData = responseData.at(endTrial).scan(@updateTrialData,trialData).subscriptable;

	trialData.isRewarded = responseData.at(response)*trialData.trialSide < 0 && trialData.isRewardedIfCorrect;
	...
	water = at(trialData.rewardSize,trialData.isRewarded.delay(0.01));  
...
function trialData = initializeTrialData(expRef, ...
    contrasts, sigmoidParamsFlip, performanceTracker,alphaPerformanceTracker, ...
    sigmoidParamsBias, biasTracker, alphaBiasTracker, ...
    rewardProb, flipRate, rewardSize, wheelGain)
	...
	trialData.contrasts = contrasts;
	trialData.trialContrast = randsample(contrasts,1);
	trialData.sigmoidParamsFlip = sigmoidParamsFlip;
	trialData.performanceTracker = performanceTracker; %don't need it here?
	trialData.alphaPerformanceTracker = alphaPerformanceTracker;
	trialData.sigmoidParamsBias = sigmoidParamsBias;
	trialData.biasTracker = biasTracker;
	trialData.alphaBiasTracker = alphaBiasTracker;
	trialData.flipRate = flipRate; %don't need it here?
	trialData.rewardProb = rewardProb;
	trialData.rewardSize = rewardSize; %don't need it here?
	trialData.wheelGain = wheelGain; %don't need it here?
	...
	% Initialize hit/miss
	trialData.isRewarded = false;
	trialData.hit = false;
	trialData.miss = false;
	trialData.randRewardProb = rand; % this is the random reward probability
	trialData.isRewardedIfCorrect = trialData.randRewardProb <= trialData.rewardProb;
	...
	% lastFlipRate = flipRate;
	            ...
	            if isempty(previousBlock) || ~isfield(previousBlock, 'trialSideValues')
	                lastTrialSide = trialData.trialSide;
	            else
	...
	if useOldParams
	    ...
	    if length(previousBlock.newTrialValues) > 200 && lastRewardSize > 2
	        trialData.rewardSize = lastRewardSize-0.1;
	    else
	        trialData.rewardSize = lastRewardSize;
	    end
    
	    trialData.flipRate = lastFlipRate;
	    trialData.performanceTracker = lastPerformanceTracker;
	    trialData.biasTracker = lastBiasTracker;
	    ...
	    if randomFlip < trialData.flipRate
	        ...
	        trialData.trialSide = lastTrialSide;
	    end
    
	else
	    % If this animal has no previous experiments, initialize performance 
	    % Initialize water reward size & wheel gain
	    trialData.flipRate = flipRate;
	    trialData.performanceTracker = performanceTracker;
	    trialData.biasTracker = biasTracker;
	    trialData.rewardSize = rewardSize;
	    trialData.wheelGain = wheelGain;
	end
end
...
% end
	...
	stimDisplacement = responseData; % this is weirdly the opposite of the main code!

	% Next contrast
	trialData.trialContrast = randsample(trialData.contrasts,1); 

	%%%% Define response type based on trial condition
	% trialData.randRewardProb = rand;
